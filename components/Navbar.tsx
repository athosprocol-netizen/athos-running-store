    ...
    <>
      ...
      >
        <div className="max-w-[1400px] mx-auto px-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 cursor-pointer group" onClick={() => setView('home')}>
             <span className={`text-4xl font-black tracking-tighter italic group-hover:text-athos-orange transition-colors duration-300 drop-shadow-sm ${logoColorClass}`}>
                ATHOS
             </span>
          </div>
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2 flex items-center gap-12">
            {navItems.map((item) => (
              <button
                ...
                className={`text-sm font-black tracking-widest uppercase transition-all hover:text-athos-orange relative group ${
                  view === item.id ? 'text-athos-orange' : textColorClass
                }`}
              >
                {item.label}
            ...
            {user?.role === 'admin' && (
                <button
                    ...
                    className={`text-sm font-black tracking-widest uppercase transition-all hover:text-athos-orange relative group ${
                    view === 'admin' ? 'text-athos-orange' : textColorClass
                    }`}
                >
                    Admin
                </button>
      ...
      <nav className={`fixed top-0 left-0 w-full z-40 transition-all duration-300 md:hidden h-16 flex items-center justify-between px-4 ${isScrolled || isMobileMenuOpen ? 'bg-white border-b border-gray-100 shadow-sm' : 'bg-[#F7F7F7]'}`}>
          ...
          <div 
              ...
              className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-center gap-1"
          >
              <Flame size={20} className="text-athos-orange fill-athos-orange" />
              <span className={`text-2xl font-black tracking-tighter italic text-athos-black`}>ATHOS</span>
          </div>
